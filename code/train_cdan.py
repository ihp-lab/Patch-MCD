...
from adaptation.cdan import ConditionalDomainAdversarialLoss
...
def main():
	...
	# Names, paths, logs
	parser.add_argument('--logger_path', default='checkpoints/cdan', help='relative path to log')
...
def domain_adaptation(opt):
	...
	discriminator = DomainDiscriminator(in_feature=opt.features_dim*opt.num_class, hidden_size=1024).to(device)
	...
	lr_scheduler = StepwiseLR(optimizer, init_lr=opt.learning_rate, gamma=0.001, decay_rate=0.75)

	threshold_list = [opt.threshold] * opt.num_class

	main_criterion = loss.Total_Loss(opt)
	domain_criterion = ConditionalDomainAdversarialLoss(
		discriminator, entropy_conditioning=opt.entropy,
		num_classes=opt.num_class, features_dim=opt.features_dim, randomized=False
    ).to(device)
...
def train(batches, n_batches, model, optimizer, lr_scheduler, main_criterion, domain_criterion, device, threshold_list, opt):
	...
	for i, train_data in enumerate(batches):
		...
		domain_loss = domain_criterion(y_s, f_s, y_t, f_t)
