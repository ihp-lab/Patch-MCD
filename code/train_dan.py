...
def domain_adaptation(opt):
	...
	optimizer = torch.optim.Adam(model.get_parameters(), lr=opt.learning_rate)

	threshold_list = [opt.threshold] * opt.num_class

	main_criterion = loss.Total_Loss(opt)
	domain_criterion = MultipleKernelMaximumMeanDiscrepancy(
		kernels=[GaussianKernel(alpha=2 ** k) for k in range(-3, 2)],
		linear=not opt.non_linear, quadratic_program=opt.quadratic_program
    )
