...
def main():
	...
	# Names, paths, logs
	parser.add_argument('--logger_path', default='checkpoints/mcd', help='relative path to log')
...
def domain_adaptation(opt):
	...
	optimizer_g = torch.optim.Adam(G.get_parameters(), lr=opt.learning_rate)
	...
	for epoch in range(opt.epochs_num):
		...
		train(	batch_iterator, n_batches, G, F1, F2, optimizer_g, optimizer_f,
				main_criterion, device, threshold_list, opt)
...
def train(	batches, n_batches, G, F1, F2, optimizer_g, optimizer_f,
			criterion, device, threshold_list, opt):
	G.train()
	F1.train()
	F2.train()
	...
	for i, train_data in enumerate(batches):
		...
		labels_s = labels_s[:,:opt.num_class].to(device)
		...
		for k in range(opt.num_k):
			...
			mcd_loss = classifier_discrepancy(y1_t, y2_t) * opt.domain_weight
			mcd_loss.backward()
			optimizer_g.step()
